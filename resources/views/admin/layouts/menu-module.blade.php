...
@if(($module['urlName'] && Route::has($module['urlName'])) || (!$module['urlName'] && moduleMenuIsVisible($module)))
    ...
    @canany($module['canany'])
        <div @if(count($module['submodules'])) data-kt-menu-trigger="click" @endif
            class="menu-item @if(count($module['submodules'])) menu-accordion @endif {{ active($module['active']) }}">
            <a href="{{ $module['urlName'] ? route($module['urlName']) : '#' }}"
                class="menu-link {{ active($module['active']) }}">
