...
<div class="d-flex flex-column gap-7 gap-lg-10">
                            ...
                            <tr>
                                <td class="text-muted">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <!--end::Svg Icon-->{{ __('Payment method') }}
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td class="text-muted">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <!--end::Svg Icon-->{{ __('Payment status') }}
                                    </div>
                                </td>
                        ...
                        <tbody class="fw-bold text-gray-600">
                                            ...
                                            <div class="symbol symbol-circle symbol-25px overflow-hidden me-3">
                                                <a href="{{ route('admin.user.show', $order->user) }}">
                                                    <div class="symbol-label">
                                                        <img loading="lazy" src="{{ $order->user->imagePreview() }}"
                                                            alt="{{ $order->user->name }}" class="w-100" />
                                                    </div>
                                                </a>
                                            </div>
                                            <!--end::Avatar-->
                                                ...
                                                <div class="symbol symbol-circle symbol-25px overflow-hidden me-3">
                                                    <div class="symbol-label">
                                                        <img loading="lazy"
                                                            ...
                                                            alt="{{ $order->address->name }}" class="w-100" />
                                                    </div>
                                                </div>
                                                <!--end::Avatar-->
                                                <!--begin::Name-->
                                                <a
                                                    class="text-gray-600 text-hover-primary">{{ $order->address->name }}</a>
                                                <!--end::Name-->
                                            @endif
                                        @endif
                                    </div>
                            ...
                            @if($order->address)
                                ...
                                <tr>
                                    <td class="text-muted">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <!--end::Svg Icon-->{{ __('Email') }}
                                        </div>
                                    </td>
                                    <td class="fw-bolder text-end">
                                        <a href="mailto:{{ $order->address->email }}"
                                            class="text-gray-600 text-hover-primary">{{ $order->address->email }}</a>
                                    </td>
                                </tr>
                                <!--end::Payment method-->
                                <!--begin::Date-->
                                            ...
                                            <span class="svg-icon svg-icon-2 me-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    ...
                                                    <path
                                                        ...
                                                        fill="gray" />
                                                    <path opacity="0.3" d="M19 4H5V20H19V4Z" fill="gray" />
                                                </svg>
                                            </span>
                                            <!--end::Svg Icon-->{{ __('Phone') }}
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td class="fw-bolder text-end"><a
                                            href="https://wa.me/+52{{ $order->address->phone }}">https://wa.me/+52{{ $order->address->phone }}</a>
                                    </td>
                                </tr>
                                <!--end::Date-->
                            @endif
    ...
    <div class="d-flex flex-column flex-xl-row gap-7 gap-lg-10">
        ...
        <div class="card card-flush py-4 flex-row-fluid overflow-hidden">
            ...
            <!--begin::Card body-->
            <div class="card-body pt-0">
                @if($order->address && $order->address->is_billing)
                    <strong>{{ __('Country') }}:</strong> {{ $order->address->state->country->name }},
                    <strong>{{ __('EIN') }}:</strong> {{ $order->address->vat }},
                    @if(($order->address->country->code ?? '') == 'MX')
                        <strong>{{ __('Fiscal regime') }}:</strong> {{ $order->address->fiscalRegime ? $order->address->fiscalRegime->description : '' }}, <br>
                        <strong>{{ __('Use cfdi') }}:</strong> {{ $order->address->useCfdi ? $order->address->useCfdi->description : '' }}, <br>
                    @endif
                    <strong>{{ __('State') }}:</strong> {{ $order->address->state->name }}, <br>
                    <strong>{{ __('Municipality') }}:</strong> {{ $order->address->municipality }},
                    <strong>{{ __('Colony') }}:</strong> {{ $order->address->colony }}, <br>
                    <strong>{{ __('Zip code') }}:</strong> {{ $order->address->zip_code }},
                    <strong>{{ __('Street') }}:</strong> {{ $order->address->street }}, <br>
                    <strong>{{ __('Between streets') }}:</strong> {{ $order->address->street_between }},
                    <strong>{{ __('Street references') }}:</strong> {{ $order->address->street_references }},
                    <br>
                    <strong>{{ __('Company name') }}:</strong> {{ $order->address->company }},
                    <strong>{{ __('To name') }}:</strong> {{ $order->address->name }}, <br>
                    <strong>{{ __('Phone') }}:</strong> {{ $order->address->phone }},
                    <strong>{{ __('Email') }}:</strong> {{ $order->address->email }}
                @else
                    {{ __('No information') }}
                @endif
        ...
        <div class="card card-flush py-4 flex-row-fluid overflow-hidden">
            <!--begin::Background-->
            <div class="position-absolute top-0 end-0 opacity-10 pe-none text-end">
                <img loading="lazy" src="{{ asset('assets/admin/media/icons/duotune/ecommerce/ecm006.svg') }}"
                    class="w-175px" />
            </div>
            <!--end::Background-->
            <!--begin::Card header-->
                    ...
                    <tbody class="fw-bold text-gray-600">
                        @foreach($order->orderProducts as $orderProduct)
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <!--begin::Thumbnail-->
                                        <a href="{{ route('admin.catalog.product.show', $orderProduct->product) }}"
                                            class="symbol symbol-50px">
                                            <span class="symbol-label"
                                                style="background-image:url({{ $orderProduct->product->imagePreview() }});"></span>
                                        </a>
                                        <!--end::Thumbnail-->
                                        <div class="ms-5">
                                            <a href="{{ route('admin.catalog.product.show', $orderProduct->product) }}"
                                            ...
                                            <div x-data="{ show: false }">
                                                <span x-on:click="show = true" x-show="show == false"><a
                                                        style="cursor: pointer;">Ver detalles de almacén</a></span>
                                                <span x-on:click="show = false" x-show="show == true"><a
                                                        style="cursor: pointer;">Ver menos</a></span>
                                                <div x-show="show == true">
                                                    @foreach($orderProduct->orderProductWarehouses as $orderProductWarehouse)
                                                        <div class="fs-7">
                                                            {{ $orderProductWarehouse->productWarehouse->name }}:
                                                            ({{ $orderProductWarehouse->quantity }}).
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                                ...
                                <!--end::Product-->
                                <!--begin::SKU-->
                                <td class="text-end">{{ $orderProduct->product->sku }}</td>
                                <!--end::SKU-->
                                <!--begin::Quantity-->
                                <td class="text-end">{{ $orderProduct->quantity }}</td>
                                <!--end::Quantity-->
                                <!--begin::Price-->
                                <td class="text-end">${{ number_format($orderProduct->price, 2) }}
                                    {{ $order->currency }}</td>
                                <!--end::Price-->
                                <!--begin::Total-->
                                <td class="text-end">${{ number_format($orderProduct->subtotal, 2) }}
                                    {{ $order->currency }}</td>
                                <!--end::Total-->
                            </tr>
                            <!--end::Products-->
                        ...
                        @if($order->tax)
                           ...
                           <tr>
                               ...
                               <td class="text-end">{{ $order->taxToString() }}</td>
                           </tr>
                           <!--end::Shipping-->
                        @endif
                        @if($order->coupon)
