<div>
    ...
    <form class="form" wire:submit.prevent="{{ $method }}">
        ...
        <div class="fv-row mb-7">
            ...
            <select wire:model="gallery.module_web_id"
                class="form-select form-select-solid @error('gallery.module_web_id') invalid-feedback @enderror">
                <option value="">{{ __('Select a option') }}</option>
                @foreach($modulesWeb as $moduleWeb)
                    <option value="{{ $moduleWeb->id }}">{{ $moduleWeb->name }}</option>
                @endforeach
            </select>
            @error('gallery.module_web_id')
                ...
                <div class="col-lg-10">
                    ...
                    <div class="dropzone dropzone-queue mb-2">
                        ...
                        <div class="dropzone-panel mb-lg-0 mb-2">
                            <a wire:loading.attr="disabled" wire:target="imagesTmp"
                            ...
                            <input wire:model="imagesTmp" type="file" class="d-none"
                                id="imagesTmp-{{ $gallery->id }}" multiple id="imagesTmp-{{ $imagesTmpInputId }}">
                        </div>
                        <!--end::Controls-->
                    </div>
                    <!--end::File-->
                    <!--begin::Hint-->
                    <span
                        class="form-text text-muted">{{ __('Try to upload no more than 10 images at a time.') }}</span>
                    <!--end::Hint-->
                </div>
                <!--end::Col-->
                @error('imagesTmp')
        ...
        <div class="text-center pt-15">
            ...
            <button wire:loading.attr="disabled" wire:target="{{ $method }}, imagesTmp" type="submit"
