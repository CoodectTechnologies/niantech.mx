                ...
                <div class="card-body pt-0">
                    <div class="d-flex flex-column gap-10">
                        ...
                        <div class="fv-row">
                            ...
                            <!--end::Label-->
                            <select wire:model="configuratorStage.type" wire:change="loadLastOrder" required
                                class="form-control mb-2 @error('configuratorStage.type') invalid-feedback @enderror">
                                <option value="">{{ __('Select a option') }}</option>
                                @foreach($this->getTypes() as $type)
                                    <option value="{{ $type }}">{{ $type }}</option>
                                @endforeach
                            </select>
                            @error('configuratorStage.type')
                                <small class="form-text text-danger" role="alert">{{ $message }}</small>
                            @enderror
                        </div>
                        <!--end::Input group-->
                        ...
                        <div class="fv-row">
                            ...
                            <!--end::Label-->
                            <input wire:model="configuratorStage.order" required type="text"
                                class="form-control mb-2 @error('configuratorStage.order') invalid-feedback @enderror"
                                placeholder="Ejem: 1" />
                            @error('configuratorStage.order')
                                <small class="form-text text-danger" role="alert">{{ $message }}</small>
                            @enderror
                        </div>
                        <!--end::Input group-->
                        ...
                        <div class="fv-row">
                            ...
                            <!--end::Label-->
                            <select required wire:model="configuratorStage.optional"
                                class="form-select mb-2 @error('configuratorStage.optional') invalid-feedback @enderror">
                                <option value="">{{ __('Select a option') }}</option>
                                <option value="1">{{ __('Optional') }}</option>
                                <option value="0">{{ __('Required') }}</option>
                            </select>
                            @error('configuratorStage.optional')
                                <small class="form-text text-danger" role="alert">{{ $message }}</small>
                            @enderror
                        </div>
                        <!--end::Input group-->
                                ...
                                <div class="col-lg-12">
                                    <div class="fv-row mb-5">
                                        ...
                                        <select wire:model="productCategoryFilter" multiple class="form-select mb-2"
                                            ...
                                            @foreach(json_decode(json_encode($productCategories)) as $categoryFhater)
                                                <option value="{{ $categoryFhater->id }}" style="font-weight: bold;">
                                ...
                                <thead>
                                    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="w-25px pe-2 text-end">
                                            <button wire:click="selectAllProducts" type="button"
                                                class="btn btn-primary">
                                                <span class="indicator-label">{{ __('Select all') }}</span>
                                                <span wire:loading wire:target="selectAllProducts"
