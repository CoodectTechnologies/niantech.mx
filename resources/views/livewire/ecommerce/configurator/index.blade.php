                                ...
                                <div class="col-lg-6">
                                    <div x-show="tab == 'tab-component' || tab == 'tab-summary'" class="row p-2">
                                        @foreach($stagesComponent as $stageComponent)
                                            ...
                                            <div wire:click.prevent="buildStage({{ $stageComponent->id }}, '{{ $this->getStageTypeComponent() }}')"
                                                x-on:click="tabComponent"
                                                class="{{ count($stagesComponent) % 2 != 0 && $loop->iteration == 1 ? 'col-12' : 'col-6' }}"
                                                style="cursor: pointer;">
                                                <div
                                                    class="mb-3 background-2 border-radius background-hover {{ $this->buildStageComponent->id == $stageComponent->id ? 'background-active' : '' }}">
                                                    @include('ecommerce.configurator.partials._stage', [
                                    ...
                                    <div x-show="tab == 'tab-addon'" class="row p-2">
                                        @foreach($stagesAddon as $stageAddon)
                                            ...
                                            <div wire:click.prevent="buildStage({{ $stageAddon->id }}, '{{ $this->getStageTypeAddon() }}')"
                                                x-on:click="tabAddon"
                                                class="{{ count($stagesAddon) % 2 != 0 && $loop->iteration == 1 ? 'col-12' : 'col-6' }}"
                                                style="cursor: pointer;">
                                                <div
                                                    class="mb-3 background-2 border-radius background-hover {{ $this->buildStageAddon->id == $stageAddon->id ? 'background-active' : '' }}">
                                                    @include('ecommerce.configurator.partials._stage', [
                                                        'stage' => $stageAddon,
                                                        'product' => $product,
                                                    ])
                        ...
                        <div class="row">
                            <div class="tab tab-nav-boxed tab-nav-underline">
                                <ul class="nav nav-tabs d-flex justify-content-center" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link" x-bind:class="tab == 'tab-component' ? 'active' : ''"
                                            x-on:click="tabComponent" href="#tab2-1">{{ __('Components') }}</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" x-bind:class="tab == 'tab-addon' ? 'active' : ''"
                                            x-on:click="tabAddon" href="#tab2-2">{{ __('Addons') }}</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" x-bind:class="tab == 'tab-summary' ? 'active' : ''"
                                            x-on:click="tabSummary" href="#tab2-3">{{ __('Summary') }}</a>
                                    </li>
                                </ul>
                                <div class="tab-content">
                                    <div class="tab-pane" x-bind:class="tab == 'tab-component' ? 'active' : ''"
                                        id="tab2-1">
                                        <div class="overflow">
                                            ...
                                            @foreach($productsComponent as $productComponent)
                                                @include('ecommerce.configurator.partials._product', [
                                                    'product' => $productComponent,
                                                    'stageType' => 'Componentes',
                                                ])
                                            @endforeach
                                        </div>
                                    </div>
                                    <div class="tab-pane" x-bind:class="tab == 'tab-addon' ? 'active' : ''"
                                        id="tab2-2">
                                        <div class="overflow">
                                            <div class="d-flex justify-content-center py-2 my-2" role="status">
                                                <div wire:loading wire:target="buildStage" class="spinner-grow"></div>
                                            ...
                                            @foreach($productsAddon as $productAddon)
                                                @include('ecommerce.configurator.partials._product', [
                                                    ...
                                                    'stageType' => 'Complementos',
                                                ])
                                            @endforeach
                                        </div>
                                    </div>
                                    <div class="tab-pane" x-bind:class="tab == 'tab-summary' ? 'active' : ''"
                                        id="tab2-3">
                                        <div class="overflow">
