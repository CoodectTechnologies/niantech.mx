                ...
                <div class="main-content post-single-content">
                    <div class="post post-grid post-single">
                        <figure class="post-media br-sm">
                            <img loading="lazy" src="{{ $post->imagePreview() }}" alt="{{ $post->name }}" width="930"
                                height="500" />
                        </figure>
                        <div class="post-details">
                            <div class="post-meta">
                                ...
                                <a class="post-comment"><i
                                        class="w-icon-comments"></i><span>{{ count($comments) }}</span>{{ __('Comments') }}</a>
                            </div>
                        </div>
                    </div>
                    <!-- End Post -->
                    ...
                    <div class="tags">
                        <label class="text-dark me-2">{{ __('Tags') }}:</label>
                        @foreach($post->blogTags as $tag)
                            <a href="{{ route('ecommerce.blog.index', ['tag' => $tag->slug]) }}"
                    ...
                    <div class="post-navigation">
                        <div class="nav nav-prev">
                            @if($previous)
                                <a href="{{ route('ecommerce.blog.show', $previous) }}"
                                    class="align-items-start text-left">
                                ...
                                <div class="post post-grid">
                                    <figure class="post-media br-sm">
                                        <a href="{{ route('ecommerce.blog.show', $postRelated) }}">
                                            <img loading="lazy" src="{{ $postRelated->imagePreview() }}"
                                                alt="{{ $postRelated->name }}" width="296" height="190"
                                    ...
                                    <div class="post-details text-center">
                                        ...
                                        <h4 class="post-title mb-3"><a
                                                href="{{ route('ecommerce.blog.show', $postRelated) }}">{{ $postRelated->name }}</a>
                                        </h4>
                                        <a href="{{ route('ecommerce.blog.show', $postRelated) }}"
                                            class="btn btn-link btn-dark btn-underline font-weight-normal">{{ __('Read more') }}<i
