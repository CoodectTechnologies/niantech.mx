            ...
            @if(!count($cart))
                <div class="text-center alert alert-primary alert-bg alert-button alert-block">
                    ...
                    <a href="{{ route('ecommerce.product.index') }}"
                        class="btn btn-primary btn-rounded">{{ __('Show products') }}</a>
                                ...
                                @foreach($getShippingZonesFreeShippingOverTo as $shippingZone)
                                            ...
                                            <h4 class="alert-title">
                                                ...
                                                <span class="">
                                                    ...
                                                    @if($shippingZone->zip_codes)
                                                        , Restringido a los codigos postales
                                    ...
                                    @else
                                        <div class="alert alert-icon alert-warning alert-bg alert-inline">
                                            <h4 class="alert-title">
                                                {{ $shippingZone->alias }} - Aumenta
                                                <strong>${{ number_format($missingAmount, 2) }}</strong> para obtener
                                                este envio gratis. <br>
                                                <span class="">
                                                    Aplica para: {{ $shippingZone->country->name }}
                                                    ( @foreach($shippingZone->states as $state)
                                                        {{ $state->name }}
                                    ...
                                    <tr class="tab-content shadow-sm">
                                        <td class="product-thumbnail">
                                            <div class="p-relative">
                                                <a href="{{ route('ecommerce.product.show', $item->model) }}">
                                                    <figure>
                                                        <img loading="lazy" src="{{ $item->options->image }}"
                                                            alt="{{ $item->name }}" width="300" height="338">
                                                    </figure>
                                                </a>
                                                <button wire:click="delete('{{ $item->rowId }}')"
                                                    wire:target="delete('{{ $item->rowId }}')"
                                                    wire:loading.class="load-more-overlay loading" class="btn btn-close"
                                                    type="submit">
                                        ...
                                        <td class="product-name">
                                            ...
                                            @if($item->options->type == 'Digital')
                                                <small class="text-muted">{{ __('Type') }}: {{ $item->options->type }}</small><br>
                                            @endif
                                            @if(isset($item->options['variant']))
                                                @foreach($item->options['variant']['options'] as $option)
                                                    <small class="text-muted">{{ $option['option_name'] }}: {{ $option['option_value'] }}</small><br>
                                                @endforeach
                                            @endif
                                        </td>
                                        <td class="product-price"><span
                                                class="amount">{{ currencySymbol() }}{{ number_format($item->price, 2) }}
                                                {{ $item->options->currency }}</span></td>
                                        <td class="product-quantity">
                                            @if(!$item->model->getIsInStock())
                                                <div
                                                    class="alert alert-warning alert-simple alert-inline show-code-action">
                                            ...
                                            @else
                                                @if(!$item->options->type || $item->options->type == 'Físico')
                                                    <div class="input-group">
                                                        <input
                                                            wire:change="update({{ $item->model->id }}, '{{ $item->rowId }}', $event.target.value)"
                                                            value="{{ $item->qty }}" class="form-control"
                                                            type="number">
                                                        <button
                                                            wire:click="update({{ $item->model->id }}, '{{ $item->rowId }}', {{ $item->qty + 1 }})"
                                        ...
                                        <td class="product-subtotal">
                                            <span
                                                class="amount">{{ currencySymbol() }}{{ number_format($item->subtotal, 2) }}
                        ...
                        <div class="cart-action mb-6">
                            <a href="{{ route('ecommerce.product.index') }}"
                                class="btn btn-dark btn-rounded btn-icon-left btn-shopping mr-auto"><i
                                    class="w-icon-long-arrow-left"></i>{{ __('Continue shopping') }}</a>
                            <button wire:click="deleteCart" type="submit"
                                class="btn btn-rounded btn-default btn-clear">{{ __('Clear cart') }}</button>
                        </div>
                        ...
                        <div class="sticky-sidebar">
                            <div class="cart-summary mb-4">
                                ...
                                <div class="cart-subtotal d-flex align-items-center justify-content-between">
                                    ...
                                    <span>{{ currencySymbol() }}{{ $subtotal }} {{ currency() }}</span>
                                </div>
                                @if(floatval(str_replace(config('cart.format.thousand_seperator'), '', Cart::instance('default')->tax())))
                                    <div class="cart-subtotal d-flex align-items-center justify-content-between">
                                        <label class="ls-25">{{ __('Tax') }}</label>
                                        <span>{{ currencySymbol() }}{{ $tax }} {{ currency() }}</span>
                                    </div>
                                    <div class="cart-subtotal d-flex align-items-center justify-content-between">
                                        <label class="ls-25">{{ __('Total') }}</label>
                                        <span>{{ currencySymbol() }}{{ Cart::total() }} {{ currency() }}</span>
                                    </div>
                                @endif
