            ...
            @else
                <div class="compare-table">
                    <div class="compare-row cols-xl-5 cols-lg-4 cols-md-3 cols-2 compare-products">
                        ...
                        @foreach($compares as $item)
                            <div class="compare-col compare-product">
                                <a wire:click.prevent="delete('{{ $item->rowId }}')"
                                    wire:target="delete('{{ $item->rowId }}')"
                                    wire:loading.class="load-more-overlay loading" href="#"
                                ...
                                <div class="product text-center">
                                    <figure class="product-media">
                                        <a href="{{ route('ecommerce.product.show', $item->model) }}">
                                            <img loading="lazy" src="{{ $item->model->imagePreview() }}"
                                                alt="{{ $item->model->name }}" width="228" height="257" />
                                        </a>
                            ...
                            <div class="compare-col compare-rating">
                                <div class="ratings-container">
                                    <div class="ratings-full">
                                        <span class="ratings"
                                            style="width: {{ $item->model->getStarsPercentageAVG() }}%;"></span>
                                        <span class="tooltiptext tooltip-top"></span>
                                    </div>
                                    <a href="{{ route('ecommerce.product.show', $item->model) }}"
                                        class="rating-reviews">({{ $item->model->comments()->validate()->count() }}
                                        {{ __('Reviews') }})</a>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="compare-row cols-xl-5 cols-lg-4 cols-md-3 cols-2 compare-category">
                        ...
                        @foreach($compares as $item)
                            <div class="compare-col compare-value">
                                {{ $item->model->productCategories()->first() ? $item->model->productCategories()->first()->name : '' }}
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="compare-row cols-xl-5 cols-lg-4 cols-md-3 cols-2 compare-brand">
                        ...
                        @foreach($compares as $item)
                            <div class="compare-col compare-value">
                                {{ $item->model->productBrand ? $item->model->productBrand->name : '' }}</div>
                        @endforeach
                    </div>
                    ...
                    <div class="compare-row cols-xl-5 cols-lg-4 cols-md-3 cols-2 compare-brand">
                        ...
                        @foreach($compares as $item)
                            <a class="btn btn-dark btn-rounded d-flex align-items-center justify-content-center"
                                href="{{ route('ecommerce.product.show', $item->model) }}">
                                <i class="fa fa-eye"></i>
                            </a>
                        @endforeach
                    </div>
