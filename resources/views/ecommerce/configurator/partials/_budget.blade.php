                                        ...
                                        @foreach($games as $game)
                                            <div class="col-6">
                                                <div class="p-3 bg-white border-radius m-1">
                                                    <img width="157" src="{{ $game->imagePreview() }}"
                                                        alt="{{ $game->name }}">
                                                </div>
                                        ...
                                        <div class="row justify-content-center">
                                            @foreach($performances as $performance)
                                                <div class="col col-lg-4 text-center">
                                                    <button wire:click="buildPerformance({{ $performance->id }})"
                        ...
                        <div class="col-lg-6">
                            <div class="row justify-content-center">
                                    ...
                                    <div class="row justify-content-center">
                                        @foreach($chipsets as $chipset)
                                            <div class="col col-lg-6 text-center">
                                                <button wire:click="buildChipset({{ $chipset->id }})"
                                ...
                                <div class="header-search hs-expanded hs-round d-md-flex input-wrapper">
                                    <button {{ !$buildBudgetBefore->exists ? 'disabled' : '' }}
                                        wire:click="buildBudget({{ $buildBudgetBefore->id }})"
                                        wire:target="buildBudget({{ $buildBudgetBefore->id }})"
                                        wire:loading.attr="disabled" class="btn btn-search btn-secondary btn-sm"
                                        type="button">
                                        <i class="w-icon-long-arrow-left"></i>
                                    </button>
                                    <input value="{{ $buildBudget->amountToString() }}" type="text" readonly
                                        class="form-control text-center" style="font-size: 2rem" />
                                    <button {{ !$buildBudgetAfter->exists ? 'disabled' : '' }}
                                ...
                                <div class="mt-5">
                                    <button wire:click="buildProductsByBudget" wire:target="buildProductsByBudget"
